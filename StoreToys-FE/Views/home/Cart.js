...
window.onload = function() {
    ...
    function updateCartItems(userId, updates) {
        const updatePromises = updates.map(update => {
            return fetch(`http://localhost/StoreToys-BE/API/cart?id=${update.id}&user_id=${userId}`, {
                method: 'PATCH',
                headers: {
	...
	function removeCartItem(userId, id, productPrice) {
		...
		.then(data => {
            ...
            // console.log(sumPrice,productPrice);
        ...
        if (event.target.classList.contains('update_cart-button')) {
            ...
            if (userId) {
                ...
                inputs.forEach(input => {
                    ...
                    if (newQuantity === 0) {
                        removeCartItem(userId, id, input.dataset.productPrice);
                    } else {
                        updates.push({ id, quantity: newQuantity });
                    }
                });
                updateCartItems(userId, updates);
            } else {
                alert('Vui lòng đăng nhập trước khi cập nhật giỏ hàng.');
            }
        }
